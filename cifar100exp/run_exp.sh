#!/bin/bash
# =============================================================================
# run_exp.sh — CIFAR-100 Noise_Nopeek+CEM(SlotAttn) sweep  (16 experiments)
#   Directly comparable to paper Table S5 "Noise_Nopeek+CEM" on CIFAR-100
...
run_exp.sh  —  CIFAR-100 Noise_Nopeek+CEM(SlotAttn) sweep
Run timestamp : ${RUN_TS}
GPU           : ${GPU_ID}

Purpose: Compare SlotAttn CEM vs GMM CEM on CIFAR-100
  ...
  Compare against: Paper Table S5 Noise_Nopeek+CEM on CIFAR-100
...
Paper baseline: Noise_Nopeek+CEM on CIFAR-100 (Table S5)
RDEOF

lm "======================================================================"
lm " run_exp.sh  |  CIFAR-100 Noise_Nopeek+CEM(SlotAttn) sweep  |  GPU ${GPU_ID}"
lm " Run dir  :  ${RUN_DIR}"
lm "======================================================================"

# ── fixed parameters (same for every experiment) ─────────────────────────────
# ── All fixed to match paper Section 5.1 for CIFAR-100 ───────────────────────
ARCH=vgg11_bn_sgm
BATCH_SIZE=128              # CEM-main CIFAR uses 128
NUM_CLIENT=1
RANDOM_SEED=125
CUTLAYER=3                  # paper "first two conv layers" = cutlayer=3 for CIFAR
BOTTLENECK=noRELU_C8S1      # paper: 8-channel bottleneck for all methods
DATASET=cifar100            # CIFAR-100 (auto-downloads on first run)
SCHEME=V2_epoch
...
#
# cutlayer=3 + vgg11_bn_sgm(feature_size=16) + noRELU_C8S1:
#   input resized to 64x64 → Conv64→MaxPool(32x32)→Conv128(32x32)→BN_C8S1
...
for ENTRY in "${EXPERIMENTS[@]}"; do
    ...
    FOLDER="saves/cifar100/${AT_REG}_cemfixed_lg${LOG_ENTROPY}_vt${VT}"
...
print('-' * W)
print("  Paper baseline: Noise_Nopeek+CEM on CIFAR-100 (Table S5)  — compare your best row")
print()
