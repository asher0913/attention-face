#!/bin/bash
# =============================================================================
# run3.sh — FaceScrub SCA_new hyperparameter sweep  (20 experiments)
#
# Usage     : bash run3.sh [GPU_ID]          (default GPU=0)
# Creates   : run3log_YYYYMMDD_HHMMSS/  in the same directory as this script
...
RUN_DIR="${SCRIPT_DIR}/run3log_${RUN_TS}"
mkdir -p "${RUN_DIR}"
MASTER_LOG="${RUN_DIR}/master.log"

# ── logging helpers ───────────────────────────────────────────────────────────
# lm   : script-level line  → stdout + master.log
# lexp : exp-level line     → stdout + expXX.log + master.log  (needs $EXP_LOG)
...
run3.sh  —  FaceScrub SCA_new hyperparameter sweep
...
summary.csv columns
  ...
  heads                number of CrossAttention heads (must divide 8)
...
Baseline (run_exp1.sh): lambd=16 noise=0.05 var_thr=0.15 loss_scale=0.5
  effective_strength=8.0   infer_ssim ≈ 0.532   (lower = better privacy)
RDEOF

lm "======================================================================"
lm " run3.sh  |  FaceScrub SCA_new sweep  |  GPU ${GPU_ID}"
lm " Run dir  :  ${RUN_DIR}"
lm "======================================================================"
...
#   EID  LAMBD  NOISE  VT  LS  SLOTS  HEADS  ITERS  WARMUP  EPOCHS
#
# HEADS must evenly divide the bottleneck channel count (8).
# Valid values: 1  2  4  8
#
# Group A (01-03): lambd sweep vs baseline
# Group B (04-07): noise + var_threshold push
# Group C (08-11): SlotAttention architecture (slots / iters)
# Group D (12-14): warmup sensitivity
# Group E (15-18): strong combined configs
# Group F (19-20): aggressive long-run best guesses
EXPERIMENTS=(
  "exp01  24  0.05  0.15  1.0   8  4  3  3  300"
  "exp02  32  0.05  0.15  1.0   8  4  3  3  300"
  "exp03  48  0.05  0.15  0.8   8  4  3  3  300"
  "exp04  24  0.08  0.20  1.0   8  4  3  3  300"
  "exp05  32  0.08  0.20  0.8   8  4  3  3  300"
  "exp06  24  0.06  0.25  1.0   8  4  3  3  300"
  "exp07  32  0.10  0.20  0.8   8  4  3  3  300"
  "exp08  24  0.05  0.15  1.0  16  4  3  3  300"
  "exp09  24  0.05  0.15  1.0   4  4  3  3  300"
  "exp10  32  0.05  0.15  1.0   8  4  5  3  300"
  "exp11  24  0.05  0.15  1.0  12  4  4  3  300"
  "exp12  24  0.07  0.20  1.0   8  4  3  1  300"
  "exp13  32  0.06  0.20  1.0   8  4  3  5  300"
  "exp14  24  0.07  0.20  1.2   8  4  3  5  300"
  "exp15  40  0.06  0.20  1.0  12  4  4  3  300"
  "exp16  36  0.07  0.25  0.8   8  4  4  5  300"
  "exp17  48  0.08  0.25  0.8   8  4  3  5  300"
  "exp18  28  0.07  0.20  1.0  10  2  4  3  300"
  "exp19  40  0.06  0.20  1.0   8  4  5  3  360"
  "exp20  36  0.06  0.25  1.2  12  4  4  5  300"
)
...
for ENTRY in "${EXPERIMENTS[@]}"; do
    ...
    read -r EID LAMBD NOISE VT LS SLOTS HEADS ITERS WARMUP EPOCHS <<< "${ENTRY}"
    ...
    FOLDER="saves/facescrub/${AT_REG}_run3_lg${LOG_ENTROPY}_vt${VT}"
    FNAME="l${LAMBD}_n${NOISE}_ep${EPOCHS}_vt${VT}_ls${LS}_sl${SLOTS}_it${ITERS}_wu${WARMUP}"
...
CFGEOF
    ...
    # ── experiment header → stdout + expXX.log + master.log ──────────────────
    {
        ...
        printf '   slots=%-3s  heads=%s  iters=%s  warmup=%s  epochs=%s\n' \
               "${SLOTS}" "${HEADS}" "${ITERS}" "${WARMUP}" "${EPOCHS}"
        printf '   log: %s\n' "${EXP_LOG}"
        printf '%.0s=' {1..72}; printf '\n'
...
#  Receives the run directory path as sys.argv[1]
# ══════════════════════════════════════════════════════════════════════════════
${PY} - "${RUN_DIR}" << 'PYEOF'
...
def parse_log(path):
    ...
    for i, line in enumerate(lines):
        ...
        # Best validation accuracy — overwrite on every match to get the final best
        m = re.search(r'Best Average Validation Accuracy is\s+([0-9.]+)', line)
        if m:
            last_acc = m.group(1)

        # Training-time MIA
        # The debug format string embeds a literal \n, so values appear on the
        # line immediately following "... averaging N times"
        if 'MIA performance Score training time is' in line:
            for j in range(i + 1, min(i + 6, len(lines))):
                nxt = lines[j].strip()
                # Values line starts with a digit (no logging-level prefix)
        ...
        # Inference-time MIA — single formatted line
        m = re.search(
...
print('-' * W)
print("  Baseline run_exp1.sh : eff=8.0  noise=0.05  vt=0.15"
      "  |  in_ssim=0.53200   (lower = better privacy)")
print()
