...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.training:
            ...
            slots = mu.clone()  # deterministic during eval/test

        attn = None  # will hold last iteration's attention weights
        for _ in range(self.num_iterations):
            ...
            attn = torch.clamp(attn, min=self.eps)        # prevent exact zeros
            attn = attn / attn.sum(dim=-1, keepdim=True)  # re-normalize
        ...
        return slots, attn  # attn shape: [B, N, S]
...
class FeatureMemoryBank:
    """Ring buffer storing recent features per class for cross-sample clustering."""
    ...
    def update(self, features, labels):
        """features: [B, D], labels: [B]. Stores detached copies."""
    ...
    def get_class_features(self, cls_idx):
        """Returns [valid_count, D] tensor for class cls_idx, or None if empty."""
    ...
    def MIA_attack(self, num_epochs, attack_option="MIA", collude_client=1, target_client=0, noise_aware=False,
        ...
        if ("MIA" in attack_option) and ("MIA_mf" not in attack_option):
            ...
            if self.dataset == "cifar10":
                ...
                input_nc=8
                input_dim=64  # TinyImageNet is 64x64
